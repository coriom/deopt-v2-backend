...
`OPTIONS_ENABLED=false`, `OPTION_RFQ_ENABLED=false`, `OPTION_EXECUTION_ENABLED=false`, `OPTION_EXECUTION_SIMULATION_ENABLED=false`, and `OPTION_EXECUTION_BROADCAST_ENABLED=false` are the safe defaults. When any options persistence gate is true and its feature is enabled, startup requires `PERSISTENCE_ENABLED=true`. Test and development code can run option series, off-chain option orders, matching, fills, option RFQs, and option execution intent signing/calldata in memory with persistence disabled. `OPTION_RFQ_QUOTE_SIGNATURE_MODE=disabled` preserves the unsigned Option RFQ V1B flow. `strict` requires each option RFQ quote to include `quote_nonce` and a valid EIP-712 `OptionRFQQuote` signature whose recovered signer equals `mm_account`. `OPTION_EXECUTION_SIGNATURE_MODE=disabled` shape-validates submitted OptionTrade signatures only; `strict` recovers buyer and seller signers from the EIP-712 digest. Option execution simulation uses `eth_call` only, requires `RPC_URL` by default when enabled, accepts an optional `OPTION_EXECUTION_SIMULATION_FROM`, and never requires a private key. Option execution broadcast requires its own opt-in flag plus the executor real-broadcast flags, `RPC_URL`, `EXECUTOR_PRIVATE_KEY`, and persisted `simulation_ok` by default.
...
Options V1D adds an HTTP/core option RFQ service on top of the option series registry and off-chain option orderbook. Option RFQ V1C connects that service to the Market Maker Gateway and adds signed market-maker option RFQ quotes: HTTP-created option RFQs are pushed to active MM sessions, MMs can submit `option_rfq_quote` over WebTransport, and quote acceptance sends best-effort MM notifications.

Option Execution Intents V1C is a separate opt-in bridge from option fills to the new Solidity `OptionMatchingEngine`. When `OPTION_EXECUTION_ENABLED=true`, matched option orderbook fills and accepted option RFQ fills create `option_execution_intents`. The backend requires an on-chain option id from `onchain_series_id` or `onchain_product_id`, EVM-address `underlying` and `settlement_asset`, whole-contract fill size conversion from `size_1e8 / 1e8`, and settlement-native premium conversion from `price_1e8` using `OPTION_EXECUTION_DEFAULT_SETTLEMENT_DECIMALS`. V1C creates signing and calldata artifacts only: it does not create execution transactions, call `/executor/broadcast`, use private keys, fabricate tx hashes, or mark option intents submitted or confirmed.
...
Option execution intent endpoints:
...
Option execution simulation V1D is enabled separately with `OPTION_EXECUTION_SIMULATION_ENABLED=true`. `POST /options/execution-intents/:intent_id/simulate` requires an existing intent, both signatures, stored calldata, a nonzero `OPTION_MATCHING_ENGINE_ADDRESS`, and `RPC_URL` when `OPTION_EXECUTION_REQUIRE_RPC_FOR_SIMULATION=true`. It performs `eth_call` to `OPTION_MATCHING_ENGINE_ADDRESS` with `data=intent.calldata`, `value=0`, optional `gas` from `OPTION_EXECUTION_SIMULATION_GAS_LIMIT`, and `from=OPTION_EXECUTION_SIMULATION_FROM` or `EXECUTOR_FROM_ADDRESS`. `OptionMatchingEngine.executeTrade` is executor-gated, so the simulated `from` address must be allowed on-chain or the call will revert. The result is stored on `option_execution_intents` as `simulation_status`, `simulation_error`, `simulation_block_number`, `simulation_revert_data`, `simulation_revert_selector`, and `simulated_at_ms`. `GET /options/execution-intents/:intent_id/simulation` returns the persisted result, or `simulation_pending` before any simulation. Option simulation never calls `/executor/broadcast`, never creates transaction rows, never submits transactions, never fabricates tx hashes, and never marks option intents submitted or confirmed.

Option execution broadcast V1I is disabled by default and exposed separately at `POST /options/execution-intents/:intent_id/broadcast`. It requires `OPTION_EXECUTION_BROADCAST_ENABLED=true`, `OPTION_EXECUTION_ENABLED=true`, `EXECUTION_ENABLED=true`, `EXECUTOR_REAL_BROADCAST_ENABLED=true`, `RPC_URL`, `EXECUTOR_PRIVATE_KEY`, and a nonzero `OPTION_MATCHING_ENGINE_ADDRESS`. By default `OPTION_EXECUTION_REQUIRE_SIMULATION_OK=true` requires the latest persisted option simulation status to be `simulation_ok`. The endpoint rejects missing calldata or buyer/seller signatures, sends only to `OPTION_MATCHING_ENGINE_ADDRESS` with `value=0`, and uses the existing transaction broadcaster/signing path. `OPTION_EXECUTION_BROADCAST_GAS_LIMIT=0` uses the existing executor gas limit; a nonzero value overrides it for option broadcast. Successful sends persist the returned transaction hash in `option_execution_transactions` and move the intent to `broadcast_submitted`. Failed sends persist `status=failed` with no tx hash and move the intent to `broadcast_failed`. Repeated calls for an already submitted intent return the existing transaction metadata and do not rebroadcast. V1I does not confirm, reconcile, index option trades, fabricate tx hashes, or call `/executor/broadcast`.
...
Option order matching and option RFQ acceptance remain off-chain. Option Execution Intents V1C adds signing/calldata artifacts for `OptionMatchingEngine`, but on-chain submission, indexer/reconciliation/confirmation, nonce sync, Greeks, IV surfaces, on-chain registry sync, settlement, and exercise remain deferred.
...
`POST /options/execution-intents/<intent_id>/broadcast` submits only calldata-ready, signed option execution intents. With the default simulation gate, the persisted simulation status must be `simulation_ok`. The endpoint stores a tx hash only after the configured broadcaster returns one, returns existing submitted transaction metadata on duplicate calls, and leaves confirmation/reconciliation/indexing out of scope.
...
- Options V1D/V1C supports manual option series registration, deterministic `option_series_id`, off-chain GTC option orders, price-time matching, fill recording/listing, cancellation, listing/filtering, aggregated option orderbook reads, HTTP/core option RFQs with off-chain RFQ fills, signed MM option RFQ quotes, MM Gateway option RFQ request/quote/accept notification messages, and opt-in option execution intents with EIP-712 `OptionTrade` signing payloads/signature storage/calldata artifacts; option on-chain submission remains deferred.
...
- Optional blockchain RPC includes manual `eth_call` simulation, opt-in indexing, and explicitly gated `eth_sendRawTransaction` broadcast for the existing perp executor. No frontend auth UI, admin write controls, WebSocket API, or option execution broadcast.
