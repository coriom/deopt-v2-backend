...
async fn bootstrap_probe_signer_identity(
    ...
    let mtls_cert = deopt_v2_backend::hybrid_v2::execution::read_pem_if_configured(
        &cfg.signer_mtls_cert_pem_path,
    )
    .map_err(|e| format!("signer bootstrap mTLS cert: {e}"))?;
    let mtls_key = deopt_v2_backend::hybrid_v2::execution::read_pem_if_configured(
        &cfg.signer_mtls_key_pem_path,
    )
    .map_err(|e| format!("signer bootstrap mTLS key: {e}"))?;
    let root_ca = deopt_v2_backend::hybrid_v2::execution::read_pem_if_configured(
        &cfg.signer_root_ca_pem_path,
    )
