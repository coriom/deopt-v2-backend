...
struct SignOptionRfqQuoteOutput {
...
fn main() -> Result<()> {
    ...
    let private_key = env::var("MM_PRIVATE_KEY").map_err(|_| {
        BackendError::Config("MM_PRIVATE_KEY is required for sign_option_rfq_quote".to_string())
    })?;
        ...
        .and_then(Value::as_str)
    {
        if !signer
            .address()
            .0
            .eq_ignore_ascii_case(&AccountId::new(mm_account.to_string()).0)
        {
    ...
    let output = SignOptionRfqQuoteOutput {
...
fn payload_path() -> Result<String> {
    ...
    Err(BackendError::Config(
        "usage: sign_option_rfq_quote --payload /path/to/option_rfq_quote_payload.json".to_string(),
    ))
}
